...
void print_usage() {
  ...
  std::cout << "schedulers: --scheduler fifo|mlfq|ml_guided --model results/runtime_linear_model.csv\n";
}
...
int main(int argc, char** argv) {
  try {
    ...
    if (mode == "benchmark") {
      const auto summaries = sched::run_all_schedulers(tasks, input_label, model_path);
    ...
    if (mode == "simulate") {
      const auto predictor = sched::load_runtime_predictor(model_path);
